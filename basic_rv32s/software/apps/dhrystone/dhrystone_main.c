  ...
  /* Main and Proc_0 in the Ada version             */
{
...
#if defined(DHRY_DEBUG_EVERY) && (DHRY_DEBUG_EVERY > 0)
      if ((Run_Index % DHRY_DEBUG_EVERY) == 0) {
        unsigned long cyc_now = (unsigned long)read_csr(mcycle);
        debug_printf("[DHRY-DBG] run=%d/%d mcycle=0x%lx Int_Glob=%d Bool_Glob=%d\n",
                     Run_Index, Number_Of_Runs, cyc_now, Int_Glob, Bool_Glob);
      }
#endif
  ...
  {
    unsigned long long user_time_u64 = (unsigned long long)(unsigned long)User_Time;
